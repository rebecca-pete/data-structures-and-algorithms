...
const hasChildrenValues = (arr, character) => {
  ...
  for (let i = 0; i < arr.length; i++) {
      if(arr[i].name === character) {
          //not sure why children[0] wouldn't work
...
//   return numCharacters;
    ...
    // let childrenResult = [];
    // let tally; 
    // let numCharacters = arr.length;
    let tally = 0;
    for (let i = 0; i < arr.length; i++){
        tally = tally + Object.entries(arr[i].children).length;
        if (Object.values(arr[i].spouse).length !== null) {
            tally = tally + Object.entries(arr[i].spouse).length;    
        }
        // tally = tally + Object.entries(arr[i].spouse).length;
        // Object.values(arr[i]).forEach(value => {
        //     childrenResult.push(arr[i].children(value[2]));
        }
        tally = tally + arr.length;
        // for (let j = 0; j < arr[i].children[j]; j++){
        //     childrenResult.push(arr[i].children[j]);
        // }
    return tally;
};
