...
const toTitleCase = (arr) => {
  ...
  for (var i = 0; i < arr.length; i++) {
    let newItem = arr[i].charAt(0).toUpperCase() + arr[i].slice(1);
    newArr.push(newItem);
  }
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  ...
  let result = [];
  for (var i = 0; i < arr.length; i++){
    let count = parseInt(arr[i].mass);//turn to num
    let lukeMass = parseInt(arr[0].mass); //turn to num
    if (count > lukeMass){
      // let count = count.toString(); //turn to string
...
const sortBy = (property, arr) => {
  ...
  if (property === 'price') {
    let result = arr.sort(function(a, b){return a.price - b.price});
    return result;
  }
  if (property === 'name') {
    let result = arr.sort(function(a, b){return a.name - b.name});
    return result;
  }
};
