...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <=str.length; i++) {
    let greetingChopped = str.slice(i);
    result.push(greetingChopped);
  }
  return result;
};
...
const wordsToCharList = (arr) => {
  // Solution code here...
  let result = [];
  let arrChopped = arr.split('');
  result.push(arrChopped);
  return result;
};
//I see an array with the desired output in testing, however, it still returns failed. Not sure why.
...
//   console.log(arr);
    let result = [];
    // let regex = \$\s*[02468]\s*,\s*)*\d*[02468]$;
    // let regex = $\d*[02468];
    arr.join(); //arr is an array of integers, this turns them into a string
    arr.split(regex); //this splits the string at evens
    // result.push(); pushes result into an array
  return result; //shows result array
};
